...
pip install safetensors#!/bin/bash

# Vérifier si un prompt a été fourni
if [ "$#" -lt 1 ]; then
    echo "Usage: ./inference.sh \"votre prompt ici\" [model_dir] [output_dir] [size]"
    exit 1
fi

PROMPT="$1"
MODEL_DIR="${2:-/workspace/full_model}"
OUTPUT_DIR="${3:-./generated_outputs}"
SIZE="${4:-832*480}"
...
python inference.py --prompt "$PROMPT" --model_dir "$MODEL_DIR" --output_dir "$OUTPUT_DIR" --size "$SIZE"
