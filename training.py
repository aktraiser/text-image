...
def check_dependencies():
    """Vérifie et installe les dépendances manquantes, notamment easydict."""
    required = ["torch", "transformers", "peft", "diffusers", "trl", "datasets", "easydict"]
    for pkg in required:
...
def load_model_and_tokenizer():
    ...
    # Importer les modules personnalisés depuis le dépôt cloné
    try:
        ...
        logger.error(f"Échec du chargement du modèle avec le pipeline personnalisé : {str(e)}")
        raise RuntimeError("Impossible de charger le modèle Wan2.1-T2V-14B. Vérifiez les dépendances et la structure du modèle.")
    
    # Vérifier que l'UNet est bien chargé
    if not hasattr(model, "unet") or model.unet is None:
        raise ValueError("L'UNet n'a pas été chargé correctement. Vérifiez le modèle.")
    
    # Configurer LoRA pour l'UNet
    ...
    # Appliquer LoRA au text encoder si présent
    ...
    # Charger le tokenizer du modèle ou un fallback
...
def wan_data_collator(examples):
    """Collateur de données adapté aux modèles de diffusion."""
    batch = {}
...
def setup_trainer(model, tokenizer, dataset):
    """Configure l'entraîneur avec les hyperparamètres."""
...
def main():
    """Orchestre l'entraînement et l'exportation."""
    check_dependencies()
